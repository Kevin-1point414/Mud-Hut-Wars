...
namespace Offline
{
    public static class Parser
    {
        ...
        {
            if (textToParse == null) return (0, -1);

            if (textToParse.Length < 2) textToParse = textToParse + " ";
            ...
            return (firstValues[firstValues.Count - 1], endPoint);
        }
        ...
        private static (float, int) ParseMathf(string textToParse, GameObject gameObject, int index)
        {
            ...
            {
                function = function + textToParse[i];
                i++;
                if (i == textToParse.Length) break;
            }
            int paramCount = 0;
